...
import type { CustomParameter, SystemId } from '../types/domain';

interface RenderFixedPoint {
  x: number;
  y: number;
  stability?: string;
}

interface RenderManifold {
  saddle_point: number[];
  plus: { points: number[][] };
  minus: { points: number[][] };
}

type OverlayParameters = Partial<Record<'a' | 'b' | 'delta' | 'h' | 'epsilon', number>>;

/**
 * Tests for the manifold rendering logic.
 * These test the pure logic of classifying which manifolds belong to repellers
 * vs saddles, mirroring the logic in SetValuedViz.tsx rendering code.
 */
describe('Manifold Classification Logic', () => {
  const classifyManifolds = (manifolds: RenderManifold[], fixedPoints: RenderFixedPoint[]) => {
    const repellerPoints = new Set<string>();
...
describe('Fixed Point Color Assignment', () => {
  ...
  const getFixedPointColor = (stability: string | null) => {
...
describe('Manifold Computation Trigger Logic', () => {
  // Mirrors the gating logic from SetValuedViz.tsx manifold computation useEffect.
  const shouldComputeManifolds = (showStable: boolean, showUnstable: boolean) => showStable || showUnstable;
...
describe('System-Specific Screenshot Metadata', () => {
  ...
  const getSystemFilePrefix = (dynamicSystem: SystemId) => {
    const prefixes: Record<SystemId, string> = {
      henon: 'henon',
      duffing: 'duffing_map',
      duffing_ode: 'duffing_ode',
      custom: 'custom',
      custom_ode: 'custom_ode',
    };
    return prefixes[dynamicSystem];
  };
